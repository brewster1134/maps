 ...
 */
router.get('/pois-with-sequence', async (_req, res) => {
    const pois = await loadPOIs();
    ...
    const poisWithSequence = pois.map((poi) => {
        const optimizedPOI = optimizedTrip.optimizedOrder.find((op) => Math.abs(op.lat - poi.lat) < 0.000001 &&
            Math.abs(op.lng - poi.lng) < 0.000001);
        if (optimizedPOI) {
            return { ...poi, sequence: optimizedPOI.sequence };
        }
        return poi;
    });
