...
import { POI, MatrixStatus, ValhallaRouteResponse } from '../types/index.js';
...
export async function calculateDistance(
  ...
  try {
    ...
    console.error(
      `Failed to calculate distance between ${poi1.name} and ${poi2.name}:`,
      error.message,
    );
    return null;
  }
 ...
 */
export async function initializeMissingMatrixEntries(
  pois: POI[],
): Promise<void> {
  ...
  for (let i = 0; i < pois.length; i++) {
    for (let j = i + 1; j < pois.length; j++) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, config.matrix.requestDelay),
      );
    }
  }
