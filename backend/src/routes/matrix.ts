 ...
 */
router.get('/matrix-status', async (_req: Request, res: Response) => {
  ...
  const calculatedPairs = pois.reduce(
    (count: number, poi1, i: number) =>
      count +
      pois.slice(i + 1).reduce((innerCount: number, poi2) => {
        const key = getMatrixKey(poi1, poi2);
        return (
          innerCount + (isMatrixEntryCalculated(matrix.distances, key) ? 1 : 0)
        );
      }, 0),
    0,
  );
  ...
  const missing: Array<{ poi1: string; poi2: string }> = [];
 ...
 */
router.post('/build-matrix', async (_req: Request, res: Response) => {
  const pois = await loadPOIs();
  if (pois.length === 0)
