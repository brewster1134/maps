...
import { POI, OptimizedPOI } from '../types/index.js';
 ...
 */
export function getMatrixKey(
  poi1: POI | OptimizedPOI,
  poi2: POI | OptimizedPOI,
): string {
 ...
 */
export function isMatrixEntryCalculated(
 ...
 */
export function countCalculatedPairs(
  pois: POI[],
  distances: Record<string, number>,
): number {
  return pois.reduce(
    (count: number, poi1: POI, i: number) =>
      count +
      pois.slice(i + 1).reduce((innerCount: number, poi2: POI) => {
